...
impl core::fmt::Display for GlApiSelection {
  fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
    ...
    show!(f, "//! * License: CC0 OR Zlib OR Apache-2.0 OR MIT");
    show!(f, "//!");
    show!(f, "//! Supported Features:");
    show!(f, "//! * `global_loader`: Include all mechanisms necessary for calling GL using global functions.");
    show!(f, "//! * `struct_loader`: Include all mechanisms necessary for calling GL as methods on a struct.");
    show!(f, "//! * `debug_trace_calls`: if cfg!(debug_assertions), any call to a GL function will `trace!` what was called and with what args.");
    show!(f, "//! * `debug_automatic_glGetError`: If cfg!(debug_assertions), this will automatically call `glGetError` after every call to any *other* GL function. If an error code occurs it's shown via `error!` along with the name of the function that had the error.");
    show!(
   f,
   ...
   "//! * `chlorine`: gets all C types from the `chlorine` crate (which is `no_std` friendly). Otherwise they will be imported from `std::os::raw`."
    ...
    show!(f, "//! * `bytemuck`: Adds support for the `bytemuck` crate, mostly in the form of `bytemuck::Zeroable` on `GlFns`.");
    show!(f, "//! * `inline`: Tags all GL calls as `#[inline]`.");
    show!(f, "//! * `inline_always`: Tags all GL calls as `#[inline(always)]`. This will effectively override the `inline` feature.");
    show!(f, "//!");
    show!(f, "//! The crate is `no_std` friendly by default, but features above can end up requiring `std` to be available.");
    if EXAMPLE_MODE {
  ...
  pub fn new_from_registry_api_extensions(
    ...
    for extension_name in extensions {
      let the_extension = reg
        ...
        .unwrap();
      assert!(
    the_extension.supported.contains(api.supported()),
    "Requested {extension_name} with api {api:?}, but it is not supported by that API.",
          ...
          "void" => match words_iter.next() {
            ...
            Some("(*") => match s.as_str() {
              ...
              "typedef void (* GLVULKANPROCNV)(void);" => {
                new = "GLVULKANPROCNV"; r#"O<extern "system" fn()>"#
              },
...
#[doc(hidden)]
pub fn {name}_load_with(get_proc_address: &mut GPA) -> bool {{
  let p = do_the_load(get_proc_address, b\"{name}\\0\");
...
impl core::fmt::Display for StructLoaderDisplayer<'_> {
  fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
   ...
   "  impl GlFns {{
    ...
    pub fn load_with(
      ...
      // Safety: the struct is nothing but `AtomicPtr` fields, which can be safely constructed with `zeroed`.
    ...
    for gl_command in self.gl_commands.iter() {
      show!(
        f,
        "  self.{short_name}_load_with(get_proc_address);",
  ...
  pub unsafe fn {short_name}(&self, {arg_name_and_type_list}){rust_return_type} {{
    ...
    {{
      if \"{name}\" != \"glGetError\" {{
        report_error_as_necessary_from(\"{short_name}\", self.GetError());
      }}
    }}
    out
  }}
  #[doc(hidden)]
  pub fn {short_name}_load_with(&self, get_proc_address: &mut GPA) -> bool {{
    let p = do_the_load(get_proc_address, b\"{name}\\0\");
