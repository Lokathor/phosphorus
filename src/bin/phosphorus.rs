...
impl core::fmt::Display for GlApiSelection {
  fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
    ...
    show!(f);
    show!(f, "// Generated by the `phosphorus` crate.");
    show!(f, "// License: CC0 OR Zlib OR Apache-2.0 OR MIT");
    show!(f);
    show!(
      f,
      "// note: you can replace this with your favorite way to get ctypes"
    );
    show!(f, "#[cfg(feature = \"chlorine\")] use chlorine::*;");
    show!(f, "");
...
fn do_the_load(
  ...
  let mut p: *mut c_void = get_proc_address(fn_name.as_ptr().cast());
...
impl core::fmt::Display for GlobalGlCommand<'_> {
  fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
    ...
    if let Some(proto_group_text) = self.gl_command.proto_group.as_ref() {
    ...
    if let Some(alias_of_text) = self.gl_command.alias_of.as_ref() {
      docs_notes_list.push_str(&format!(
        "/// * alias of: [`{alias_of_text}`]\n",
        alias_of_text = alias_of_text,
      ));
    }
    if let Some(vec_equivalent_text) = self.gl_command.vec_equivalent.as_ref() {
      docs_notes_list.push_str(&format!(
...
pub unsafe fn {name}({arg_name_and_type_list}){rust_return_type} {{
  ...
  {{
    trace!(\"calling {name}({arg_debug_list});\", {arg_name_list});
  }}
...
pub fn {name}_load_with(get_proc_address: &mut dyn FnMut(*const c_char) -> *mut c_void) -> bool {{
  ...
  let p = do_the_load(get_proc_address, FN_NAME, &{atomic_ptr_name});
